...
def simulated_hardening_utils(matrix, data, free, filled, groups_empty_space, teachers_empty_space, subjects_order, result, index):
    ...
    for i in range(iter_count):
        ...
        for j in range(len(data.classes) // 4):
            ...
            mutate_ideal_spot(matrix_utils, data_utils, index_class, free_utils, filled_utils, groups_empty_space_utils, teachers_empty_space_utils,
                              subjects_order_utils)
        _, _, new_cost_groups = empty_space_groups_cost(groups_empty_space_utils)
        _, _, new_cost_teachers = empty_space_teachers_cost(teachers_empty_space_utils)
        new_cost = new_cost_groups  + new_cost_teachers
        # if free_hour(matrix) == -1:
        #     new_cost += 1
        ...
        if new_cost < curr_cost or rt <= math.exp((curr_cost - new_cost) / t):
            ...
            teachers_empty_space_utils = copy.deepcopy(old_teachers_empty_space)
            subjects_order_utils = copy.deepcopy(old_subjects_order)
        if i % 10 == 0:
            print('Iteration: {:4d} | Average cost: {:0.8f}'.format(i, curr_cost))
    
    result[index] = [curr_cost, matrix_utils, free_utils, filled_utils, groups_empty_space_utils, teachers_empty_space_utils, subjects_order_utils]
...
def main():
    ...
    for i in range(5):
        ...
        for i in range(len(threads)):
            threads[i] = Thread(target=simulated_hardening_utils, args=(matrix, data, free, filled, groups_empty_space, teachers_empty_space, subjects_order, results, i))
            threads[i].start()
        ...
        matrix, free, filled, groups_empty_space, teachers_empty_space, subjects_order = results[index][1:]
